...
export default function Console() {
  ...
  const [purpose, setPurpose] = useState("Bounded AI workflow");
  ...
  async function refresh(next = tab) {
    ...
    if (next === "Overview") setSummary(await call(base + "/usage"));
      ...
      const map: Record<string, string> = {Requests:"requests", Contracts:"spend-contracts", Groups:"groups", Providers:"provider-connections", Applications:"applications", Keys:"virtual-keys", Budgets:"budgets", Audit:"audit-events", Notifications:"notifications"};
  ...
  return <div className="shell">
    <aside><a className="brand" href="/" aria-label={`${brand.name} home`}><img src={brand.mark} alt="" width={42} height={42}/><span>{brand.name.toLowerCase()}</span></a><p className="eyebrow">WORKSPACE CONTROL</p>
      {["Overview","Contracts","Requests","Groups","Providers","Applications","Keys","Budgets","Notifications","Audit"].map(item => <button className={tab === item ? "nav active" : "nav"} key={item} disabled={!tenant || busy} onClick={() => act(() => refresh(item))}>{item}</button>)}
    ...
    <main><header><div><p className="eyebrow">ORGANISATION / {tenant ? tenant.slice(0,8) : "GET STARTED"}</p><h1>{tenant ? tab : "Your AI, under control."}</h1></div><span className="badge">MOCK MODE · NO PAID CALLS</span></header>
      ...
      {tenant && tab === "Contracts" && <section className="panel"><p className="eyebrow">AI SPEND CONTRACT</p><h2>Reserve a bounded workflow envelope</h2><p>The full maximum is reserved before execution. Parallel steps, retries, and other keys cannot exceed or consume this contract’s allocation.</p><label>Contract name<input value={name} onChange={e => setName(e.target.value)} placeholder="Resolve support ticket"/></label><label>Business purpose<input value={purpose} onChange={e => setPurpose(e.target.value)}/></label><label>Application ID<input value={application} onChange={e => setApplication(e.target.value)}/></label><label>Virtual key ID<input value={parent} onChange={e => setParent(e.target.value)}/></label><label>Maximum cost (USD micro-units)<input type="number" min="1" value={budget} onChange={e => setBudget(e.target.value)}/></label><label>Maximum tokens<input type="number" min="1" value={maxTokens} onChange={e => setMaxTokens(e.target.value)}/></label><label>Maximum steps<input type="number" min="1" value={maxSteps} onChange={e => setMaxSteps(e.target.value)}/></label><button disabled={busy || !name || !application || !parent} onClick={() => {if (confirm(`Reserve $${(Number(budget)/1e6).toFixed(6)} for up to ${maxSteps} workflow steps? The capacity remains reserved until this contract is closed or revoked.`)) act(async () => {const row=await call(base+"/spend-contracts","POST",{name,purpose,application_id:application,key_id:parent,max_cost_microusd:Number(budget),max_tokens:Number(maxTokens),max_steps:Number(maxSteps),allowed_models:["org-balanced"],allowed_tools:[],data_region:"local",duration_seconds:3600}); setContractToken(row.token); await refresh(); setNotice("Spend contract created and capacity reserved.");});}}>Create spend contract</button></section>}
      {contractToken && <section className="panel secret"><h2>Copy the contract token now</h2><p>This signed, scoped token is displayed once. It expires automatically and only works with its bound virtual key and application.</p><code>{contractToken}</code><button onClick={() => setContractToken("")}>I have saved it — dismiss</button></section>}
      {tenant && tab === "Contracts" && <section className="panel"><h2>Contract receipt and lifecycle</h2><label>Contract ID<input value={contractId} onChange={e => setContractId(e.target.value)}/></label><div className="actions"><button disabled={busy || !contractId} onClick={() => act(async () => setDetail(await call(base+`/spend-contracts/${contractId}`)))}>View receipt</button><button disabled={busy || !contractId} onClick={() => act(async () => {setDetail(await call(base+`/spend-contracts/${contractId}/close`,"POST"));await refresh();})}>Close and settle</button><button disabled={busy || !contractId} onClick={() => {if(confirm(`Revoke contract ${contractId}? New steps stop immediately; uncertain allocations remain reserved.`)) act(async()=>{await call(base+`/spend-contracts/${contractId}/revoke`,"POST");setDetail(await call(base+`/spend-contracts/${contractId}`));await refresh();});}}>Revoke</button></div>{detail && <pre>{JSON.stringify(detail,null,2)}</pre>}</section>}
